...
export default function ListeningPractice({id,user,data,reload,onBack}:{id:string;user:User;data:SessionData;reload:()=>Promise<void>;onBack:()=>void}) {
 ...
 return <>
   ...
   {data.session.progress&&<div className="card lesson-progress"><strong>ฟังเข้าใจด้วยตัวเอง {data.session.progress.independent_conversations}/2 รอบ</strong><div className="lesson-progress-track" role="progressbar" aria-label="ความคืบหน้าการฟังท้ายบท" aria-valuenow={data.session.progress.percent} aria-valuemin={0} aria-valuemax={100}><div style={{width:`${data.session.progress.percent}%`}}/></div><small>ตอบตามความหมายโดยไม่เปิดคำใบ้ครบ 2 รอบ จะจบให้อัตโนมัติ</small></div>}<ErrorMessage message={error}/>{notice&&<p className="notice" role="status">{notice}</p>}
